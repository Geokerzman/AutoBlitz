    ...
    public function __construct(){
      ...
      public function index($page = 1)
      {
          ...
          if ($brand !== null || $model !== null || $year !== null) {
              ...
              // Иначе использовать метод getPosts для неотфильтрованных постов
      ...
      public function add()
{
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        // $content = file_get_contents('../public/img/back.jpg');
...
// header('Content-Type: image/jpg');
// die($content = file_get_contents($target_dir  . 'back.jpg'));
// echo( $content);
// die(PUBROOT);
  ...
  if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
    ...
    echo "Sorry, there was an error uploading your file.";
      ...
      // }
        ...
        if(!$isError){
        // if (empty($data['title_err']) && empty($data['body_err']) && empty($data['brand_err']) && empty($data['model_err']) && empty($data['description_err']) && empty($data['year_err'])) {
